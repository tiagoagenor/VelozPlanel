...
/** .NET: app-console mínimo (HttpListener) que serve a página "site em construção"
 *  na :80. Compilado UMA vez no boot (o container é a imagem SDK) → funciona em
 *  3.0→11 sem minimal-hosting. Program.cs e .csproj vão em base64 (à prova de
 *  aspas); o .csproj tem `__TFM__` trocado pela versão real no supervisor. A
...
// .csproj com `__TFM__` (trocado por net<v> / netcoreapp<v> no supervisor). AssemblyName
// fixo = vp-placeholder → roda `dotnet .../vp-placeholder.dll`.
const DOTNET_CSPROJ_B64 =
  "";
...
function cmdFor(runtime: RuntimeSpec, startupScript?: string | null): string[] {
  ...
  if (runtime.kind === "dotnet") {
    // .NET: supervisor. A imagem do ambiente é a SDK → compila UMA vez o placeholder
    // (HttpListener, /opt/vp-ph/app) para o env "nascer vivo" sem código. Depois, a
    // cada subida escolhe: comando avançado (/.vp-dotnet-cmd, de VP_DOTNET_CMD base64)
    // → DLL publicada (acha pelo *.runtimeconfig.json em /app) → placeholder.
    // ASPNETCORE_URLS/HTTP_PORTS forçam a :80. TFM = net<v> (≥5) ou netcoreapp<v> (3.x).
    // NB: a saída do build NÃO pode ser um dir ancestral da fonte (quebra o CS glob) →
    // fonte em /opt/vp-ph/src, saída em /opt/vp-ph/app.
    const script =
      setup +
      `touch /.veloz-env-capable; mkdir -p /app; ` +
      `trap 'kill "\$VPPID" 2>/dev/null; exit 0' TERM INT; ` +
      `[ -f /.vp-dotnet-cmd ] || { [ -n "\$VP_DOTNET_CMD" ] && printf '%s' "\$VP_DOTNET_CMD" | base64 -d > /.vp-dotnet-cmd; }; ` +
      `if [ ! -f /opt/vp-ph/app/vp-placeholder.dll ]; then ` +
      `mkdir -p /opt/vp-ph/src; ` +
      `printf '%s' '${DOTNET_PROGRAM_B64}' | base64 -d > /opt/vp-ph/src/Program.cs; ` +
      `V="\${VP_RUNTIME_VERSION:-8.0}"; MJ="\${V%%.*}"; ` +
      `if [ "\$MJ" -ge 5 ] 2>/dev/null; then TFM="net\$V"; else TFM="netcoreapp\$V"; fi; ` +
      `printf '%s' '${DOTNET_CSPROJ_B64}' | base64 -d | sed "s/__TFM__/\$TFM/" > /opt/vp-ph/src/vp-placeholder.csproj; ` +
      `( cd /opt/vp-ph/src && dotnet build vp-placeholder.csproj -c Release -o /opt/vp-ph/app --nologo ) > /opt/vp-ph/build.log 2>&1 || true; ` +
      `fi; ` +
      `while :; do ` +
      LOAD_ENV +
      ...
      `else dotnet /opt/vp-ph/app/vp-placeholder.dll & fi; ` +
