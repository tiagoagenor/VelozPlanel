...
export default function EnvLogsPage() {
  ...
  const start = React.useCallback(
    async (tailN: number) => {
      ...
      let res: Response;
      try {
        res = await fetch(api.envLogsStreamUrl(id, tailN), { credentials: "include", cache: "no-store", signal: ac.signal });
      } catch {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-1.5 text-sm text-text2">
            Linhas
            <select value={tail} onChange={(e) => setTail(Number(e.target.value))} className="rounded-lg border border-border bg-surface px-2 py-1.5 text-sm text-text outline-none focus:border-brand-strong">
              {TAIL_OPTIONS.map((n) => <option key={n} value={n}>{n}</option>)}
            </select>
          </label>
          {live ? (
            <Button variant="outline" size="sm" onClick={() => abortRef.current?.abort()}><Pause size={15} aria-hidden="true" /> Pausar</Button>
          ) : (
            <Button variant="outline" size="sm" onClick={() => void start(tail)}><Play size={15} aria-hidden="true" /> {status === "ended" ? "Retomar" : "Reconectar"}</Button>
          )}
          <Button variant="outline" size="sm" onClick={() => void start(tail)} aria-label="Recarregar"><RotateCw size={15} aria-hidden="true" /> Recarregar</Button>
          <Button variant="ghost" size="sm" onClick={() => setLines([])} disabled={lines.length === 0}><Trash2 size={15} aria-hidden="true" /> Limpar</Button>
        </div>
      </div>
      ...
      <Card className="p-0">
        <div ref={boxRef} onScroll={onScroll} className="h-[60vh] overflow-auto rounded-xl bg-[#0d1117] p-4 font-mono text-[12.5px] leading-relaxed text-[#d1d5db]">
          {status === "connecting" && lines.length === 0 ? (
            <p className="flex items-center gap-2 text-[#9ca3af]"><Loader2 size={14} aria-hidden="true" className="animate-spin" /> Conectando aos logs…</p>
          ) : lines.length === 0 ? (
      ...
      <p className="text-xs text-text3">
        {live
          ...
          : status === "ended"
            ...
            : "As últimas linhas de stdout/stderr do seu app."}
